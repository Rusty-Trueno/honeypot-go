...
func NewKCP(addr string, setting *KcpSetting) (*UDPListener, error) {
	...
	listener := &UDPListener{connChan: make(chan *UDPMakeSession), quitChan: make(chan struct{}), sock: udpSocket, readBuffer: make([]byte, ReadBufferSize*2), sessions: make(map[string]*UDPMakeSession), setting: setting}
	go listener.loop()
	return listener, nil
}
...
func DialKcpTimeout(addr string, timeout int) (*UDPMakeSession, error) {
	return DialKcpTimeoutWithSetting(addr, timeout, DefaultKcpSetting(), 0, 0, false, false)
}

func DialKcpTimeoutWithSetting(addr string, timeout int, setting *KcpSetting, ds, ps int, comp, confuse bool) (*UDPMakeSession, error) {
	bReset := false
	if timeout < 5 {
		bReset = true
		timeout = 5
	...
	if bReset {
		fmt.Printf("timeout should in [5, 255], force reset timeout to", timeout)
	}
	udpAddr, err := net.ResolveUDPAddr("udp", addr)
	if err != nil {
		return nil, err
	...
	if _err != nil {
		fmt.Printf("dial addr fail", _err.Error())
		return nil, _err
	}
	session := &UDPMakeSession{readBuffer: make([]byte, ReadBufferSize*2), do: make(chan Action), do2: make(chan Action), quitChan: make(chan struct{}), recvChan: make(chan KcpCache), processBuffer: make([]byte, ReadBufferSize), closeChan: make(chan struct{}), encodeBuffer: make([]byte, 7), checkCanWrite: make(chan (chan struct{})), xor: setting.Xor}
	...
	if ds != 0 && ps != 0 {
		...
		if er != nil {
			fmt.Printf("set fec error:", er.Error())
		} else {
			fmt.Printf("set fec ok:", ds, ps)
		}
	}
	if comp {
	...
	code := session.doAndWait(func() {
		...
		if status == ResetAck {
			...
			fmt.Printf("pipe version not eq,%d.%d=>%d.%d", mainV, subV, _mainV, _subV)
			return 1
		}
		if status != FirstACK {
			fmt.Printf("recv not FirstACK", status)
			return -1
		} else {
	...
	if code != 0 {
		fmt.Printf("handshakefail with code", code)
...
func udp_output(buf []byte, _len int32, kcp *ikcp.Ikcpcb, user interface{}) int32 {
	...
	//fmt.Printf("send udp", _len, c.remote.String())
	c.output(buf[:_len])
	return 0
}
		...
		if err == nil {
			...
			if bHave {
				if session.status == "ok" {
					if session.remote.String() == from.String() && (n >= int(ikcp.IKCP_OVERHEAD) || session.compressCache != nil) {
						...
						if n <= 7 || session.compressCache == nil {
							...
							if _er != nil {
								fmt.Printf("decompress fail", _er.Error())
								//go session.Close()
								//don't close pipe, just drop this data
								continue
							}
							buf = _b
							//fmt.Printf("decompress", n, len(_b))
						}
				...
				if status != FirstSYN {
					...
					fmt.Printf("invalid package,reset", from, status)
					continue
				}
				sessionId := GetId("udp")
				...
				if fec & ^(1<<7) != 0 {
					...
					if er != nil {
						fmt.Printf("set fec error:", fec, er.Error())
					} else {
						fmt.Printf("set fec ok:", (int(fec)>>8)&0xff, int(fec&^(1<<7)&0xff))
					}
				}
				if int(fec)&(1<<7) != 0 {
					session.compressCache = make([]byte, zappy.MaxEncodedLen(ReadBufferSize))
			...
			//fmt.Printf("debug out.........")
		} else {
			e, ok := err.(net.Error)
			if !ok || !e.Timeout() {
				fmt.Printf("recv error", err.Error(), from)
...
func (l *UDPListener) remove(addr string) {
	fmt.Printf("listener remove", addr)
	l.sessionsLock.Lock()
...
out:
	...
	t.Stop()
	if code > 0 {
		fmt.Printf("handshake fail,got code", code)
	}
	return
}
...
func (session *UDPMakeSession) writeTo(b []byte) {
	if session.compressCache != nil && len(b) > 7 {
		...
		if er != nil {
			fmt.Printf("compress error", er.Error())
			go session.Close()
			return
		}
		//fmt.Printf("compress", len(b), len(enc))
...
func (session *UDPMakeSession) output(b []byte) {
	if session.fecW == nil {
		...
		if session.fecSendC >= uint(session.fecDataShards) {
			...
			if er != nil {
				//fmt.Printf("wocao encode err", er.Error())
				go session.Close()
				return
			}
			for i := session.fecDataShards; i < session.fecDataShards+session.fecParityShards; i++ {
				...
				//fmt.Printf("output udp id", id, i, _len, len(_info))
				//} else {
				//	fmt.Printf("drop output udp id", id, i, _len, len(_info))
				//}
			}
			session.fecWCacheTbl = nil
			session.fecSendC = 0
			session.fecSendL = 0
			session.fecWriteId++
			//fmt.Printf("flush id", id)
		}
		//fmt.Printf("output sn", c.fecWriteId, c.fecSendC, _len)
	}
}

func (session *UDPMakeSession) serverDo(s string) {
	go func() {
		//fmt.Printf("prepare handshake", session.remote)
		select {
	...
	go func() {
		...
		for {
			...
			case s := <-session.handShakeChan:
				//fmt.Printf("process handshake", session.remote)
				status, arg, arg2 := makeDecode([]byte(s), session.xor)
				switch session.status {
				case "init":
					if status != FirstSYN {
						fmt.Printf("status != FirstSYN, reset", session.remote, status)
					...
					if _mainV != mainV || _subV != subV {
						...
						fmt.Printf("pipe version not eq,kickout,%d.%d=>%d.%d", mainV, subV, _mainV, _subV)
						return
					}
					session.status = "firstack"
				...
				case "firstack":
					if status != SndSYN {
						fmt.Printf("status != SndSYN, nothing", session.remote, status)
						/*
							if status == FirstSYN {
		...
		go func() {
			...
			for {
				...
				if session.remote.String() == from.String() {
					//fmt.Printf("===", n, len(session.compressCache))
		...
		for {
			...
			case <-ping:
					...
					if session.fecR != nil {
						...
						for id, info := range session.fecRCacheTbl {
							if curr >= info.overTime {
								...
								//fmt.Printf("timeout after del", id, len(c.fecRCacheTbl))
							}
						}
					}
					if forceWait > 0 {
				...
				if time.Now().Unix() > session.overTime {
					fmt.Printf("overtime close", session.LocalAddr().String(), session.RemoteAddr().String())
					go session.Close()
				} else {
			...
			case <-recoverChan:
				...
				for _, r := range waitList {
					//fmt.Printf("recover writing data")
					select {
					case r <- struct{}{}:
					case <-session.quitChan:
					}
			...
			case c := <-session.checkCanWrite:
				...
				if ikcp.Ikcp_waitsnd(session.kcp) > dataLimit {
					//fmt.Printf("wait for data limit")
					...
					if !fastCheck {
						...
						f = func() {
							...
							//fmt.Printf("fast check!", n, len(waitList))
							if n <= dataLimit/2 {
								select {
								case <-session.quitChan:
									fmt.Printf("recover writing data quit")
								case recoverChan <- struct{}{}:
								}
							} else {
								updateF(20)
					...
					if session.fecR != nil {
						...
						if count >= session.fecDataShards {
							...
							if bNeedRebuild {
								...
								if er != nil {
									//fmt.Printf("2Reconstruct fail, close pipe", count, session.fecDataShards, session.fecParityShards, er.Error())
									//break //broken data, may be should be closed, now just keep input
								} else {
									//fmt.Printf("Reconstruct ok, input", id)
									for i := 0; i < session.fecDataShards; i++ {
										if _, have := markTbl[i]; !have {
											_len := int(tbl.bytes[i][0]) | (int(tbl.bytes[i][1]) << 8)
											ikcp.Ikcp_input(session.kcp, tbl.bytes[i][7:], int(_len))
											//fmt.Printf("fec input for mark ok", i, id, _len)
										}
									}
								}
							}
							delete(session.fecRCacheTbl, id)
							//fmt.Printf("after del", id, len(c.fecRCacheTbl))
							if session.fecRecvId <= id {
								session.fecRecvId = id + 1
							}
						}
		...
		case action := <-session.do:
			...
			case "closebegin":
				...
				time.AfterFunc(time.Millisecond*500, func() {
					//fmt.Printf("close over, step3", session.LocalAddr().String(), session.RemoteAddr().String())
			...
			case "recv":
				...
				case CloseBack:
					//A call from B
					//fmt.Printf("recv back close, step2", session.LocalAddr().String(), session.RemoteAddr().String())
					go session.DoAction("closeover")
				case Close:
					if session.closed {
						break
					...
					if session.status != "ok" {
						...
						//fmt.Printf("recv remote close, step1", session.LocalAddr().String(), session.RemoteAddr().String())
						buf := make([]byte, 7)
						go session.DoWrite(makeEncode(buf, CloseBack, 0, 0, session.xor))
						time.AfterFunc(time.Millisecond*500, func() {
							//fmt.Printf("close remote over, step4", session.LocalAddr().String(), session.RemoteAddr().String())
							if session.closed {
								return
							}
...
func (session *UDPMakeSession) _Close(bFirstCall bool) {
	...
	//session.wait.Wait()
	go func() {
		//fmt.Printf("pipe begin close", session.LocalAddr().String(), session.RemoteAddr().String())
		if bFirstCall {
		...
		//fmt.Printf("pipe end close", session.id)
...
func (session *UDPMakeSession) Write(b []byte) (n int, err error) {
	...
	if session.confuseSeed <= 0 {
		...
		copy(data[3:], b)
		//fmt.Printf("try send", len(data), sendL)
		//copy(data[3+sendL:], b)
	}
...
func (session *UDPMakeSession) Read(p []byte) (n int, err error) {
	...
	//fmt.Printf("real recv", l, string(b[:l]))
	if n == -1 {
