...
func (_self *UdpForward) DoUdpForward(srcAddr string, destAddr string) error {
	...
	if err != nil {
		fmt.Printf("ResolveUDPAddr ", srcAddr, " 出错：", err)
		return err
	}

	_self.DestAddr, err = net.ResolveUDPAddr("udp", destAddr)
	if err != nil {
		fmt.Printf("ResolveUDPAddr ", destAddr, " 出错：", err)
		return err
	}
	...
	if err != nil {
		fmt.Printf("启动UDP监听 ", srcAddr, " 出错：", err)
		return err
	}
...
func (_self *UdpForward) forwardHandler(data []byte, addr *net.UDPAddr) {
	...
	if found {
		...
		if err != nil {
			fmt.Printf("udp-forwader: failed to dial:", err)
			return
		}
